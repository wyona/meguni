 ...
 */
public class MSNParser extends Parser {
    private Category log = Category.getInstance(MSNParser.class);

    private String sampleFilename = "msn.xml";
        ...
        try {
			...
			String searchResult = searchEndOfResult(in, position);
                        if (searchResult != null) {
                            //log.error("Search result: " + searchResult);
    ...
    private Result parseResultString(String searchResultString) {
        ...
        String lastModified = searchResultString.substring(searchResultString.indexOf("<pubDate>") + 9, searchResultString.indexOf("</pubDate>"));
        try {
